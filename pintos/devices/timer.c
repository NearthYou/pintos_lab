...
/* 8254 Programmable Interval Timer(PIT)가 초당 PIT_FREQ번
   interrupt를 발생시키도록 설정하고, 해당 interrupt를 등록한다. */
void
timer_init (void) {
	/* 8254 입력 주파수를 TIMER_FREQ로 나눈 값이며,
	   가장 가까운 값으로 반올림한다. */
...
timer_calibrate (void) {
	...
	/* loops_per_tick을 타이머 tick 하나보다 아직 작은
	   가장 큰 2의 거듭제곱 값으로 근사한다. */
...
/* OS가 부팅된 이후 지난 타이머 tick 수를 반환한다. */
int64_t
timer_ticks (void) {
...
/* THEN 이후 지난 타이머 tick 수를 반환한다.
   THEN은 timer_ticks()가 반환했던 값이어야 한다. */
int64_t
...
/* 짧은 지연을 구현하기 위해 단순한 loop를 LOOPS번 반복한다.

   코드 정렬이 시간 측정에 큰 영향을 줄 수 있으므로 NO_INLINE으로
   표시한다. 이 함수가 여러 위치에서 서로 다르게 inline되면 결과를
   예측하기 어려워진다. */
...
real_time_sleep (int64_t num, int32_t denom) {
	/* NUM/DENOM초를 타이머 tick으로 변환하되, 내림한다.
	...
	ASSERT (intr_get_level () == INTR_ON);
	if (ticks > 0) {
		/* 최소 하나의 완전한 타이머 tick을 기다린다.
		   CPU를 다른 process에 양보할 수 있도록 timer_sleep()을 사용한다. */
		timer_sleep (ticks);
	} else {
		/* 그렇지 않으면 tick보다 짧은 시간을 더 정확히 맞추기 위해
		   busy-wait loop를 사용한다. overflow 가능성을 피하려고
		   분자와 분모를 1000으로 줄인다. */
		ASSERT (denom % 1000 == 0);
